...
- Halaman `/claim` kini sudah tidak menggunakan request OTP email. Pengguna melakukan registrasi awal menggunakan NRP + password kuat (kombinasi huruf, angka, dan karakter khusus), lalu login dengan kredensial yang sama untuk melanjutkan ke pembaruan profil claim.
- `AuthContext` menormalkan role direktorat (`DITBINMAS`, `DITINTELKAM`, `DITSAMAPTA`, `DITLANTAS`, `BIDHUMAS`, `DIREKTORAT`) ke bentuk kanonik untuk menghitung `effectiveRole`/`effectiveClientType`. Kombinasi `client_type: DIREKTORAT` + role `DITINTELKAM` dipertahankan sebagai `effectiveClientType: "DIREKTORAT"`, sedangkan exception khusus `DITSAMAPTA` + `BIDHUMAS` tetap dipaksa ke `effectiveClientType: "ORG"` dan `effectiveRole: "BIDHUMAS"`.【F:cicero-dashboard/context/AuthContext.tsx†L31-L326】
