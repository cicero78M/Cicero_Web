...
- Fungsi ini digunakan di:
  ...
  - Direktori user dan rekap absensi kini menambahkan filter khusus role **operator**: daftar user dibatasi pada role operator dan `client_id` login. Jika data rekap tidak memuat atribut role, hook melakukan lookup `getUserDirectory` untuk membentuk daftar `user_id`/NRP/NIP operator sebelum memfilter hasil rekap.
