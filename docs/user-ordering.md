...
- Fungsi ini digunakan di:
  ...
  - Halaman User Insight (`cicero-dashboard/app/user-insight/page.jsx`) tidak lagi melakukan fan-out lookup profil klien saat role direktorat. Label chart memakai field `nama_client`/`client_name` dari data user dan hanya memakai profil aktif (`client_id` login) sebagai fallback, sehingga tidak ada pemanggilan profil per user.
  ...
  - Hook `useTiktokCommentsData` kini memprioritaskan `effectiveRole` dan `effectiveClientType` yang sudah dinormalisasi pada konteks autentikasi sebelum jatuh ke hasil `getDashboardStats` maupun profil klien. Nilai efektif ini kemudian dipakai saat menentukan jalur direktorat, pemilihan `dashboardClientId`, dan filtrasi direktori agar klien remap seperti **DITSAMAPTA BIDHUMAS** tetap diperlakukan sebagai ORG. Nilai `effectiveClientType` yang `null` diubah ke `undefined` agar helper `getUserDirectoryFetchScope` tetap konsisten menerima tipe yang valid.
  - Direktori user dan rekap absensi kini menambahkan filter khusus role **operator**: daftar user dibatasi pada role operator dan `client_id` login. Direktori user membaca atribut role dari field tunggal (`role`, `user_role`, `roleName`) maupun koleksi (`roles`, `user_roles`), lalu tetap menampilkan operator dengan `client_id` kosong agar data personil operator tidak terlewat. Jika data rekap tidak memuat atribut role, hook melakukan lookup `getUserDirectory` untuk membentuk daftar `user_id`/NRP/NIP operator sebelum memfilter hasil rekap.
  - Tombol **Rekap User** pada halaman `cicero-dashboard/app/users/page.jsx` kini tidak lagi mengikuti toggle Ditbinmas **"Hanya ..."**. Rekap tetap menghitung seluruh user yang lolos filter direktorat, status, dan pencarian agar personel dari kesatuan lain tidak hilang saat toggle aktif.
