...
- Ringkasan eksekutif kini diambil langsung dari endpoint backend `/api/executive-summary` lewat helper `getExecutiveSummary` (`cicero-dashboard/utils/api.ts`). Filter yang dikirim mencakup `month` (YYYY-MM), `periode=bulanan`, `client_id`, `start_date`, `end_date`, serta `scope/role/regional_id` mengikuti hak akses Ditbinmas. Untuk data konten, helper `getInstagramPosts` sekarang mengarah ke rute Cicero_V2 baru `/api/cicero_v2/instagram/posts` dan meneruskan konteks `scope`, `role`, dan `regional_id` agar backend menyesuaikan otorisasi lintas satker.
- Respons yang dikonsumsi UI memuat `monthLabel`, `summaryMetrics` (label, value, suffix, change), `highlights`, `engagementByChannel` (reach + engagementRate), `audienceComposition`, `contentTable`, `platformAnalytics`, dan `narratives` (overview/dashboard/instagram/tiktok). Semua bidang ini langsung dipetakan ke kartu, chart, dan tabel pada halaman `/executive-summary`. Payload posting Instagram dari endpoint baru dinormalisasi ke bidang `like_count`, `comment_count`, `share_count`, `save_count`, `view_count`, `reach`, `engagement_rate`, serta tanggal `published_at/activityDate` sehingga perhitungan tren bulanan & mingguan tetap konsisten.
