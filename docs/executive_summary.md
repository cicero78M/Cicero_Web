...
- Pemanggilan konten Instagram pada halaman Executive Summary otomatis memuat ulang ketika profil, role, scope, atau `regional_id` dari konteks autentikasi berubah sehingga kueri `/api/instagram/posts` selalu membawa parameter cakupan Ditbinmas (contoh: `scope=ALL&regional_id=JATIM`).【F:cicero-dashboard/app/executive-summary/page.jsx†L3645-L3653】【F:cicero-dashboard/app/executive-summary/page.jsx†L4017-L4054】
- Ringkasan kinerja bulanan tidak lagi bergantung pada payload `summaryMetrics` backend. Frontend menghitung metrik utama (total likes, komentar, engagement, jumlah konten, serta delta vs bulan sebelumnya) melalui helper `getMonthlyPlatformRecap` yang memanggil empat endpoint: `getRekapLikesIG`, `getRekapKomentarTiktok`, `getInstagramPosts`, dan `getTiktokPosts`. Setiap permintaan meneruskan `client_id`, `role`, `scope`, `regional_id`, serta rentang tanggal (`start_date`/`end_date` dan `tanggal_mulai`/`tanggal_selesai`) yang dihasilkan dari filter bulan aktif dan bulan pembanding. Hasil agregasi dibentuk menjadi narasi ringkas dan daftar highlight oleh utilitas `buildMonthlyPerformanceNarrative` pada `/executive-summary`, sehingga kartu metrik dan narasi mengandalkan rekap likes/komentar dan daftar konten terkini.【F:cicero-dashboard/utils/api.ts†L1337-L1442】【F:cicero-dashboard/app/executive-summary/page.jsx†L191-L258】【F:cicero-dashboard/app/executive-summary/page.jsx†L270-L356】【F:cicero-dashboard/app/executive-summary/page.jsx†L2829-L3040】【F:cicero-dashboard/app/executive-summary/page.jsx†L5384-L5512】
- Kontribusi Kanal (Reach · Engagement Rate) mengambil reach dari konten Instagram/TikTok yang sudah difilter ke rentang bulan aktif, lalu memakai rekap likes Instagram dan rekap komentar TikTok sebagai sumber interaksi utama sebelum menghitung engagement rate per kanal, sehingga chart tidak kosong ketika payload `engagementByChannel` backend absen.【F:cicero-dashboard/app/executive-summary/page.jsx†L2829-L3040】【F:cicero-dashboard/app/executive-summary/page.jsx†L5446-L5512】
- Normalisasi konten TikTok pada helper `getMonthlyPlatformRecap` kini membaca payload berbentuk array maupun objek (`data` atau `posts`) sehingga jumlah konten bulanan, delta periode, dan agregasi engagement menghitung gabungan konten Instagram dan TikTok alih-alih hanya IG.【F:cicero-dashboard/utils/api.ts†L1476-L1526】【F:cicero-dashboard/app/executive-summary/page.jsx†L244-L294】【F:cicero-dashboard/app/executive-summary/page.jsx†L2829-L3040】
- Bagian **Narasi Per Kanal**, **Kontribusi Kanal**, dan **Komposisi Audiens** menyusun narasi otomatis ketika backend belum mengirim teks, berdasarkan jumlah konten per platform, jangkauan tertinggi per kanal, serta segmen audiens teratas pada periode aktif. Narasi ditampilkan berdampingan dengan kartu Ringkasan Kinerja Bulanan dan chart bar/pie untuk memastikan informasi kontribusi kanal dan audiens tetap terbaca pengguna.【F:cicero-dashboard/app/executive-summary/page.jsx†L244-L294】【F:cicero-dashboard/app/executive-summary/page.jsx†L3266-L3458】【F:cicero-dashboard/app/executive-summary/page.jsx†L5446-L5575】
...
- Respons yang dikonsumsi UI kini ditoleransi dalam format camelCase maupun snake_case: bidang seperti `monthLabel`/`month_label`, `summaryMetrics`/`summary_metrics`, `engagementByChannel`/`engagement_by_channel`, `audienceComposition`/`audience_composition`, `contentTable`/`content_table`, dan `platformAnalytics`/`platform_analytics` semuanya dinormalisasi oleh `normalizeExecutiveSummaryPayload` di `app/executive-summary/normalizers.js`. Untuk `engagementByChannel`, payload dapat berupa array maupun peta objek (mis. `{ instagram: {...}, tiktok: {...} }`) dan akan dipetakan menjadi deretan `{ channel, reach, engagementRate }` agar chart kontribusi kanal dan narasi otomatis tetap terisi ketika backend mengirim map. Hasil normalisasi memuat label periode, metrik ringkasan (label, value, suffix, change/delta), highlights, komposisi audiens, top konten, analitik platform, serta narasi (overview/dashboard/instagram/tiktok) yang dipetakan langsung ke kartu, chart, dan tabel `/executive-summary`. Payload posting Instagram dari endpoint baru tetap dinormalisasi ke bidang `like_count`, `comment_count`, `share_count`, `save_count`, `view_count`, `reach`, `engagement_rate`, serta tanggal `published_at/activityDate` sehingga perhitungan tren bulanan & mingguan konsisten.【F:cicero-dashboard/app/executive-summary/normalizers.js†L66-L116】【F:cicero-dashboard/app/executive-summary/normalizers.js†L118-L347】
- Jumlah konten TikTok pada Ringkasan Kinerja Bulanan kini disaring berdasarkan tanggal publikasi/aktivitas (`published_at`, `create_time`, `activityDate`, dst.) agar hanya konten dalam rentang bulan aktif maupun bulan pembanding yang dihitung pada total dan delta volume konten.【F:cicero-dashboard/app/executive-summary/sharedUtils.js†L5-L31】【F:cicero-dashboard/app/executive-summary/page.jsx†L2828-L2902】
