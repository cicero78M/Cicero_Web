...
- Ringkasan eksekutif kini diambil dari endpoint Cicero_V2 terbaru `/api/cicero_v2/executive-summary/monthly` melalui helper `getExecutiveSummary` (`cicero-dashboard/utils/api.ts`). Permintaan selalu menyertakan `month`/`period` (YYYY-MM), `period_scope=monthly`, rentang tanggal (`start_date`/`end_date` hasil `getMonthDateRange`), identitas `client_id`, serta konteks akses Ditbinmas (`scope`, `role`, `regional_id`) agar backend menghitung rekap sesuai kewenangan. Untuk data konten, helper `getInstagramPosts` tetap mengarah ke rute `/api/cicero_v2/instagram/posts` dan meneruskan `scope`, `role`, serta `regional_id` agar otorisasi lintas satker tetap dijaga.
