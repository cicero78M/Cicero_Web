...
- Client-side navigation stores the most recent non-public pathname in `localStorage` as `last_pathname`. Public paths such as `/`, `/login`, `/login-update`, `/claim`, and `/reposter/login` are ignored.
