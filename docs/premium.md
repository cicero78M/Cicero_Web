...
- Guard kini menunggu kombinasi `hasResolvedPremium` **dan** `premiumTierReady` dari `AuthContext` (diset setelah tier berhasil diekstraksi dari profil) sebelum melakukan redirect, sehingga tier tidak dievaluasi sebagai non-premium ketika profil masih dimuat. Ketika fetch profil gagal, hook menampilkan toast error sebagai fallback dan tidak mengarahkan ke **/premium** hingga evaluasi tier berhasil; redirect hanya terjadi jika `isPremiumTierAllowedForAnev(premium_tier)` mengembalikan `false`.
