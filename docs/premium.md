...
- Guard kini menunggu kombinasi `hasResolvedPremium` **dan** indikator tier (nilai `premiumTier` yang terisi atau flag `premiumTierReady`) sebelum melakukan redirect, sehingga tier tidak dievaluasi sebagai non-premium ketika profil masih dimuat. `hasResolvedPremium` mengikuti `premiumTierReady` agar status resolusi baru dianggap selesai setelah proses ekstraksi tier tuntas.
