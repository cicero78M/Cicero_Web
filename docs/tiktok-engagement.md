...
## Perubahan hook
`useTiktokCommentsData` kini menormalisasi payload `/api/tiktok/rekap-komentar` agar field komentar yang dikirim backend (mis. `comments`/`comment_count`) tetap dikonsumsi sebagai `jumlah_komentar`, termasuk fallback username, metadata client/divisi, serta fallback `label → nama` saat `chartData` tidak menyediakan nama. Hook juga membaca `summary.totalPosts` ketika tersedia agar total tugas komentar konsisten dengan rekap terbaru, serta menyiapkan fallback ringkasan dari root payload jika blok `summary` kosong (mendukung alias snake_case seperti `sudah_users_count` atau `total_posts`). Penggunaan `DEFAULT_INSIGHT_TABS` tidak mengubah alur salin rekap; tampilan tetap mengikuti pola insight lintas platform.

Mulai pembaruan ini, normalisasi user pada hook menyimpan field `status`, `completionPercentage`, `missingComments`, dan `total_konten` bila backend menyediakannya. Field tersebut dipakai oleh rekap untuk memprioritaskan status/performa dari payload (misalnya `status` atau `completionPercentage`), sekaligus menandai kondisi `no_posts` ketika `total_konten` bernilai 0 sebelum jatuh ke perhitungan lokal agar status tabel dan ringkasan selaras dengan backend.

Mulai update ini, jalur direktorat maupun saat scope **all** memakai helper agregasi `fetchDitbinmasAbsensiKomentarTiktok`. Helper tersebut mengambil `dashboard stats` untuk menghitung total post TikTok, lalu melakukan fan-out `getRekapKomentarTiktok` berdasarkan daftar `client_id` yang berasal dari `getUserDirectory`. Metadata `client_name` diperkaya lewat `getClientNames` dengan fallback mapping dari direktori agar setiap baris rekap tetap menampilkan nama satker. Pola ini dipilih supaya rekap direktorat tetap konsisten dengan Instagram (data tugas dari stats + daftar personel dari rekap per client), sekaligus menjaga filter scope **client** vs **all** tetap deterministik di tingkat hook.

Ringkasan komentar TikTok memprioritaskan payload `summary` dari backend bila field `sudahUsersCount`, `kurangUsersCount`, `belumUsersCount`, `noUsernameUsersCount`, dan `totalPosts` tersedia serta valid. Nilai tersebut hanya dipakai ketika jumlahnya konsisten dengan jumlah personel di tabel (hasil filter/sort hook) supaya `totalUser` dan `totalTiktokPost` tetap selaras dengan data yang ditampilkan. Jika salah satu field kosong atau tidak konsisten, hook kembali menghitung ringkasan secara lokal dari daftar personel yang dipakai tabel.

Sumber `totalTiktokPost` mengikuti urutan prioritas: (1) `summary.totalPosts` dari `getRekapKomentarTiktok` bila tersedia dan valid, (2) fallback ke `dashboard stats` bila `summary.totalPosts` kosong. Untuk fallback stats, hook memastikan request memakai `client_id` yang sama dengan rekap (atau `client_id` direktorat yang konsisten ketika role direktorat dipaksa ke Ditbinmas), sehingga jumlah post tidak tertukar antar satker. Jika `summary.totalPosts` valid dan > 0, hook menahan nilai ini agar `totalTiktokPost` tidak jatuh ke 0 akibat fallback yang kosong.

Pemilihan sumber `users` di `extractRekapPayload` memprioritaskan `chartData` ketika array tersebut memiliki metrik komentar (mis. `comments`, `comment_count`, `jumlah_komentar`, atau `missingComments`). Jika `data`/`users` dipakai, hook memastikan ia mengandung metrik komentar; bila tidak, fallback kembali ke `chartData` agar tabel rekap tidak kehilangan angka komentar. Saat identitas personel (nama, title, client) berada di `data` sementara metrik komentar ada di `chartData`, keduanya digabung lewat kunci stabil (`user_id`, `nrp/nip`, atau `label/nama`) sehingga daftar user tetap lengkap sekaligus mempertahankan angka komentar yang akurat.
