...
- Tabel rekap personel memakai layout standar: header lengket beraksen biru/indigo, kontrol pencarian + dropdown **Tampilkan status** (Semua, Sudah, Kurang, Belum, Username Kosong) di kanan atas, baris bergaya zebra dengan badge status (Sudah/Kurang/Belum/Tanpa Username/Tidak ada posting), serta pagination kompak yang konsisten di Instagram maupun TikTok.
...
## Perubahan hook
`useTiktokCommentsData` kini menormalisasi payload `/api/tiktok/rekap-komentar` agar field komentar yang dikirim backend (mis. `comments`/`comment_count`) tetap dikonsumsi sebagai `jumlah_komentar`, termasuk fallback username serta metadata client/divisi. Hook juga memetakan key root baru (`usersCount`, `totalPosts`, `sudahUsersCount`, `kurangUsersCount`, `belumUsersCount`, `usersWithCommentsCount`, `usersWithoutCommentsCount`, `noUsernameUsersCount`) ke `summary.totalUsers`, `summary.totalPosts`, dan `summary.distribution` ketika backend mengirim response `{ success: true, ... }`, sehingga layer UI tetap membaca data ringkasan dari struktur `summary`. Seluruh data chart serta ringkasan rekap TikTok Engagement Insight kini bersumber dari response endpoint ini (dengan parameter `role`, `scope`, dan `regional_id`), termasuk pembacaan `summary.totalPosts`, `summary.totalUsers`, dan distribusi status (`summary.distribution.sudah/kurang/belum/noUsername/noPosts`). Tabel **Rekap Personel TikTok** kini selalu bersumber dari `payload.data` ketika `payload.data` berbentuk array (meskipun kosong), dan hanya fallback ke `payload.users`/`dataPayload.users` jika `payload.data` tidak ada atau bukan array; data chart tetap diturunkan dari daftar user yang sama agar pembagian divisi/satfung konsisten tanpa memakai `payload.chartData`. Jika summary tidak dikirim, hook menghitung ulang ringkasan dari daftar `users` yang sama agar tetap konsisten tanpa fallback ke dashboard stats. Pemetaan chart kini menambahkan anotasi tipe pada entri user agar build TypeScript strict tidak gagal pada callback `map`. Penggunaan `DEFAULT_INSIGHT_TABS` tidak mengubah alur salin rekap; tampilan tetap mengikuti pola insight lintas platform.
- Untuk role **operator**, filter rekap TikTok kini langsung memakai `client_id` asli dari profil login sehingga daftar personel tidak lagi dikosongkan oleh penyaringan berbasis direktori atau role tambahan.
- Saat scope direktorat disetel ke `client`, data chart kini diturunkan dari daftar user yang sudah difilter `client_id` sehingga grafik hanya menampilkan personel sesuai direktorat yang dipilih.
- TikTok Engagement Insight kini mendukung **pemilihan client direktorat per satker** (seperti Instagram): selector client tampil pada tab **Dashboard Insight** dan **Rekap Detail** ketika akun login bertipe DIREKTORAT, lalu memfilter ringkasan, quick insight, grafik (`ChartBox`), serta tabel `RekapKomentarTiktok` berdasarkan client satker yang dipilih.
- Kontrak hook `useTiktokCommentsData` kini menambahkan field `clientOptions` berisi daftar client ter-normalisasi dengan shape konsisten `{ client_id, nama_client }`. Sumber data diambil berurutan dari payload rekap (`clients`, `directory`, `client_directory`, `clientDirectory`, `meta/metadata/summary.clients`) dan otomatis di-deduplicate; jika backend tidak mengirim daftar client, hook membuat fallback dari data user (`client_id`, `nama_client`, `client_name`) agar selector client direktorat tetap muncul.
- Filter `scope=client` tetap diterapkan di level hook untuk menjaga ringkasan rekap sesuai satker login, sedangkan filter tambahan `selectedClientId` dipertahankan di level view menggunakan `clientOptions` sehingga pengguna bisa drill-down per satker tanpa memecah kontrak summary utama dari hook.
