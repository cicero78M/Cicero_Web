...
- Format file output dibangun oleh util terpusat `cicero-dashboard/utils/engagementChartExport.js`: `<filePrefix>-<grouping>-<yyyy-mm-dd>.jpg`.
  - Profil platform minimal yang disediakan: `instagram` (Instagram Engagement Insight) dan `tiktok` (TikTok Engagement Insight).
  - `filePrefix` diambil dari profil platform (`instagram-engagement-direktorat` / `tiktok-engagement-direktorat`).
...
- Opsi `toJpeg` kini terpusat dalam konstanta standar (`quality`, `pixelRatio`, `backgroundColor`, `cacheBust`) agar konsisten lintas chart.
...
- Label sukses toast mengikuti profil platform agar pesan unduhan tidak hardcoded.
