...
4. Untuk role selain Ditbinmas, hook mengambil profil klien guna menentukan apakah akun termasuk direktorat atau ORG. Klien direktorat diberikan dropdown lingkup yang bisa di-toggle antara `scope: "client"` (rekap dinormalisasi ke `client_id` login) dan `scope: "all"` (agregat direktorat penuh), sementara klien ORG tidak pernah melihat kontrol lingkup dan selalu dibatasi pada `client_id` login agar operator hanya melihat satkernya sendiri. Flag kesadaran ORG (`isOrgClient`) dan `isDirectorate` diteruskan dari hook sehingga komponen UI tetap selaras dengan implementasi ORG-aware.【F:cicero-dashboard/hooks/useInstagramLikesData.ts†L97-L214】【F:cicero-dashboard/app/likes/instagram/page.jsx†L1-L160】
