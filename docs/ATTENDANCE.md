...
5. Untuk role **operator**, hook selalu memakai `client_id` login untuk `getDashboardStats`, `getClientProfile`, dan `getRekapLikesIG`. Mode direktorat/ditbinmas dinonaktifkan, sehingga kontrol lingkup tidak muncul dan data sosial media hanya berasal dari satker operator tersebut. Daftar rekap juga difilter agar hanya menampilkan personel ber-role operator pada `client_id` login; jika rekap tidak membawa atribut role, hook melakukan lookup direktori user untuk mencocokkan `user_id`/NRP/NIP operator sebelum merender ringkasan.【F:cicero-dashboard/hooks/useInstagramLikesData.ts†L90-L336】
