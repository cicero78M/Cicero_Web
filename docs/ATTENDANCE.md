...
## Dokumen turunan terkait export rekap

- Untuk spesifikasi alur pengguna, mekanisme teknis DOM capture, standar header wajib, penanganan error, acceptance criteria QA, serta batasan kompatibilitas fitur **Download Tabel JPG** pada tab rekap Instagram, rujuk [`docs/instagram-engagement-table-jpg-export.md`](./instagram-engagement-table-jpg-export.md).
...
5. Untuk role **operator**, hook tetap menggunakan `client_id` login saat meminta rekap dan hanya melakukan penyaringan role ketika tipe klien efektif bukan **ORG** (mengacu pada `effectiveClientType` yang dinormalisasi). Penyaringan user operator hanya dilakukan ketika payload rekap sudah membawa atribut role; jika tidak tersedia maka seluruh daftar rekap ditampilkan apa adanya demi menjaga sumber data tetap tunggal dari `/api/insta/rekap-likes`. Untuk kombinasi **ORG + role direktorat/ditbinmas**, daftar user tidak lagi difilter di frontend sehingga rekap mengikuti payload penuh dari server, dan ringkasan dihitung ulang agar konsisten dengan daftar yang ditampilkan.【F:cicero-dashboard/hooks/useInstagramLikesData.ts†L261-L626】
