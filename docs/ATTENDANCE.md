...
4. Untuk role selain Ditbinmas, hook mengambil profil klien guna menentukan apakah akun termasuk direktorat atau ORG. Klien direktorat diberikan dropdown lingkup: ketika `scope: "client"`, rekap lintas satker hasil agregasi dinormalisasi kembali ke `client_id` login, sedangkan `scope: "all"` menampilkan agregat direktorat penuh. Klien ORG melewati dropdown ini dan selalu dibatasi pada `client_id` login agar operator hanya melihat satkernya sendiri.【F:cicero-dashboard/hooks/useInstagramLikesData.ts†L97-L214】【F:cicero-dashboard/app/likes/instagram/page.jsx†L1-L160】
