...
function ContentItem({ item, platform }: {
  item: ClaimPendingContentItem; platform: Platform;
}) {
...
function PlatformGroup({ platform, content }: { platform: Platform; content: ClaimPendingPlatformContent }) {
  ...
  return <section aria-label={`${details.label} - ${details.action}`} className="space-y-3">
    ...
    {!content.username_available ? <p className="rounded-2xl bg-amber-50 p-4 text-sm text-amber-700">Username {details.label} belum terdaftar. Tambahkan username platform pada profil untuk memantau aktivitas.</p>
      ...
      : <ul className="grid gap-3 sm:grid-cols-2">{content.items.map((item, index) => <ContentItem key={item.shortcode || item.video_id || `${platform}-${index}`} item={item} platform={platform} />)}</ul>}
  </section>;
}
