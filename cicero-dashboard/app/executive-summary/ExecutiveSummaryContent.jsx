...
export default function ExecutiveSummaryPage() {
  ...
  const [isDetailedAnalysisEnabled, setIsDetailedAnalysisEnabled] = useState(false);
  ...
  useEffect(() => {
    if (!isAuthorized || isDetailedAnalysisEnabled) return undefined;

    const enableDetailedAnalysis = () => setIsDetailedAnalysisEnabled(true);
    if (typeof window !== "undefined" && "requestIdleCallback" in window) {
      const idleId = window.requestIdleCallback(enableDetailedAnalysis, { timeout: 1800 });
      return () => window.cancelIdleCallback?.(idleId);
    }

    const timeoutId = window.setTimeout(enableDetailedAnalysis, 900);
    return () => window.clearTimeout(timeoutId);
  }, [isAuthorized, isDetailedAnalysisEnabled]);
  ...
  useEffect(() => {
    if (!isDetailedAnalysisEnabled) {
      setMonthlyPerformanceState((prev) => ({ ...prev, loading: false }));
      return undefined;
    }
    if (!token || !clientId) {
  ...
  useEffect(() => {
    ...
    const loadUserInsight = async () => {
      if (!isDetailedAnalysisEnabled) {
        setUserInsightState((prev) => ({ ...prev, loading: false }));
  ...
  return (
    <div className="space-y-8">
      ...
      {!isDetailedAnalysisEnabled ? (
        <section className="rounded-3xl border border-sky-100 bg-white p-6 shadow-sm" aria-live="polite">
          <div className="flex items-center gap-4">
            <span className="h-10 w-10 animate-pulse rounded-full bg-sky-100" aria-hidden />
            <div>
              <h2 className="font-semibold text-neutral-navy">Menyiapkan analisis lengkap</h2>
              <p className="mt-1 text-sm text-neutral-slate">Ringkasan utama sudah dapat dibaca. Grafik, detail personel, dan konten dimuat bertahap saat browser siap.</p>
            </div>
          </div>
        </section>
      ) : (
      ...
      <section
        ...
        className="space-y-6 rounded-3xl border border-brand-100/60 bg-gradient-to-br from-white via-brand-50/60 to-collab-50/60 p-6 shadow-[0_20px_45px_rgba(29,107,135,0.12)] [content-visibility:auto] [contain-intrinsic-size:900px]"
      >
      ...
      <section
        ...
        className="space-y-6 rounded-3xl border border-sky-100 bg-white p-6 shadow-md [content-visibility:auto] [contain-intrinsic-size:1100px]"
      >
      ...
      <section
        className="relative overflow-hidden rounded-[36px] border border-sky-200/70 bg-gradient-to-br from-sky-50 via-indigo-50/70 to-emerald-50/70 p-8 shadow-[0_32px_68px_rgba(16,63,95,0.16)] [content-visibility:auto] [contain-intrinsic-size:1400px]"
      ...
      <section
        ...
        className="rounded-3xl border border-brand-100/60 bg-white/95 p-6 shadow-[0_20px_45px_rgba(29,107,135,0.12)] [content-visibility:auto] [contain-intrinsic-size:700px]"
      >
        <h2 className="text-sm font-semibold uppercase tracking-[0.35em] text-brand-600">
      ...
      <section
        ...
        className="rounded-3xl border border-brand-100/60 bg-white/95 p-6 shadow-[0_20px_45px_rgba(29,107,135,0.12)] [content-visibility:auto] [contain-intrinsic-size:400px]"
      >
        <h2 className="text-sm font-semibold uppercase tracking-[0.35em] text-brand-600">
