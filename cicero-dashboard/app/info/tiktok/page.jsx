...
export default function TiktokInfoPage() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        const username =
          clientData.client?.client_tiktok?.replace(/^@/, "") ||
          clientData.client_tiktok?.replace(/^@/, "") ||
  ...
  async function handleCompare(e) {
    ...
    try {
      ...
      setCompareStats({
        ...
        avgComments,
        avgViews,
        totalPosts: infoData?.video_count ?? infoData?.post_count ?? infoData?.media_count,
  ...
  const stats = [
    ...
    { title: "Total Posts", value: info?.video_count ?? info?.post_count ?? info?.media_count },
    { title: "Total Likes", value: info?.heart_count ?? info?.total_likes },
        ...
        {compareStats && (
          <div className="bg-white p-4 rounded-xl shadow flex flex-col gap-4">
            ...
            <div className="flex-1">
              <InstagramCompareChart
                client={{
                  ...
                  avgViews,
                  engagementRate,
                  totalPosts: info?.video_count ?? info?.post_count ?? info?.media_count,
            ...
            <Narrative>
              ...
              {`Total posting sebanyak ${info?.video_count ?? info?.post_count ?? info?.media_count}. `}
              ...
              {`Total posting ${compareStats.totalPosts || 0}.`}
            </Narrative>
          </div>
        )}
      </div>
