...
type AggregatorCandidate = {
  entry: any;
  parent?: AggregatorCandidate;
};

type AggregatorNormalizedData = {
  igProfile: any | null;
  igPosts: any[];
  tiktokProfile: any | null;
  tiktokPosts: any[];
};

const AGGREGATOR_CLIENT_ID_PATHS = [
  "client_id",
  "clientId",
  "clientID",
  "client.id",
...
const AGGREGATOR_USER_ID_PATHS = [
  "user_id",
  "userId",
  "userID",
  "user.id",
...
const normalizeClientIdValue = (value?: string) =>
  value ? value.trim().toUpperCase() : "";

const normalizeUserIdValue = (value?: string) =>
  value ? value.trim().toUpperCase() : "";
...
const pickNestedValue = (source: any, path: string) => {
...
const pickFirstAvailable = (entry: any, paths: string[]) => {
  for (const path of paths) {
    const value = pickNestedValue(entry, path);
...
const pickIdentifierFromEntry = (entry: any, paths: string[]) => {
  for (const path of paths) {
    const candidate = pickNestedValue(entry, path);
...
const pickIdentifierFromCandidate = (
  candidate: AggregatorCandidate,
  paths: string[],
) => {
  let current: AggregatorCandidate | undefined = candidate;
  while (current) {
    const identifier = pickIdentifierFromEntry(current.entry, paths);
...
const normalizePostsArray = (value: any): any[] => {
  if (!value) return [];
...
const normalizeAggregatorEntity = (entry: any): AggregatorNormalizedData => {
  const igProfile = pickFirstAvailable(entry, INSTAGRAM_PROFILE_PATHS);
  const igPostsCandidate = pickFirstAvailable(entry, INSTAGRAM_POSTS_PATHS);
  const tiktokProfile = pickFirstAvailable(entry, TIKTOK_PROFILE_PATHS);
  const tiktokPostsCandidate = pickFirstAvailable(entry, TIKTOK_POSTS_PATHS);

  return {
    igProfile: igProfile ?? null,
    igPosts: normalizePostsArray(igPostsCandidate),
    tiktokProfile: tiktokProfile ?? null,
    tiktokPosts: normalizePostsArray(tiktokPostsCandidate),
  };
};

const hasAggregatorSocialData = (entry: any) => {
  if (!entry || typeof entry !== "object") return false;
  const normalized = normalizeAggregatorEntity(entry);
  return (
    Boolean(normalized.igProfile) ||
    Boolean(normalized.tiktokProfile) ||
    normalized.igPosts.length > 0 ||
    normalized.tiktokPosts.length > 0
  );
};

const collectAggregatorCandidates = (root: any): AggregatorCandidate[] => {
  const candidates: AggregatorCandidate[] = [];
  const seen = new Set<any>();

  const pushCandidate = (entry: any, parent?: AggregatorCandidate) => {
    if (!entry || typeof entry !== "object" || seen.has(entry)) return;
    const candidate: AggregatorCandidate = { entry, parent };
    seen.add(entry);
    ...
    collections.forEach((collection) => {
      if (Array.isArray(collection)) {
        collection.forEach((item) => pushCandidate(item, candidate));
      } else if (collection && typeof collection === "object") {
        pushCandidate(collection, candidate);
      }
    });

    if (entry.data && typeof entry.data === "object") {
      if (Array.isArray(entry.data)) {
        entry.data.forEach((item: any) => pushCandidate(item, candidate));
      } else {
        pushCandidate(entry.data, candidate);
      }
    }
  };
...
const matchesAggregatorContext = (
  candidate: AggregatorCandidate,
  context: { clientId: string; userId: string; role: string },
) => {
  if (!context.clientId) return false;
  const candidateClientId = normalizeClientIdValue(
    pickIdentifierFromCandidate(candidate, AGGREGATOR_CLIENT_ID_PATHS),
  );
  if (!candidateClientId || candidateClientId !== context.clientId) {
    return false;
  }

  if (context.role === "ditbinmas" && context.userId) {
    const candidateUserId = normalizeUserIdValue(
      pickIdentifierFromCandidate(candidate, AGGREGATOR_USER_ID_PATHS),
    );
...
const normalizeAggregatorData = (
  rawData: any,
  context: AggregatorContext,
): AggregatorNormalizedData => {
  const root = rawData && typeof rawData === "object" ? rawData : {};
  const candidates = collectAggregatorCandidates(root);
  const normalizedContext = {
    clientId: normalizeClientIdValue(context.clientId),
    userId: normalizeUserIdValue(context.userId),
    role: context.role ? context.role.trim().toLowerCase() : "",
  };

  const matched = candidates.find((candidate) =>
    matchesAggregatorContext(candidate, normalizedContext),
  );

  const fallback =
    matched || candidates.find((candidate) => hasAggregatorSocialData(candidate.entry));

  const targetEntry = fallback?.entry ?? root;
  const normalized = normalizeAggregatorEntity(targetEntry);

  return {
    igProfile: normalized.igProfile,
    igPosts: normalized.igPosts,
    tiktokProfile: normalized.tiktokProfile,
    tiktokPosts: normalized.tiktokPosts,
  };
};

export default function DashboardPage() {
  const { token, profile, clientId, userId, role } = useAuth();
  ...
  const normalizedRole = useMemo(() => {
    const fallbackRole =
      ...
      (typeof window !== "undefined"
        ...
        : "");
    return fallbackRole ? fallbackRole.trim().toLowerCase() : "";
  }, [role]);
  ...
  const resolvedClientId = useMemo(() => {
    ...
    if (typeof window !== "undefined") {
      values.push(window.localStorage.getItem("client_id") ?? "");
    }

    const selected = pickFirstNonEmpty(values);
    return selected ? selected.trim().toUpperCase() : "";
  }, [clientId, profile]);
  ...
  const resolvedUserId = useMemo(() => {
    ...
    if (typeof window !== "undefined") {
      values.push(window.localStorage.getItem("user_id") ?? "");
    }
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        const normalized = normalizeAggregatorData(data, {
          clientId: resolvedClientId,
          userId: normalizedRole === "ditbinmas" ? resolvedUserId : undefined,
          role: normalizedRole,
        });

        if (cancelled) return;

        setIgProfile(normalized.igProfile ?? null);
        setIgPosts(Array.isArray(normalized.igPosts) ? normalized.igPosts : []);
        setTiktokProfile(normalized.tiktokProfile ?? null);
        setTiktokPosts(
          Array.isArray(normalized.tiktokPosts) ? normalized.tiktokPosts : [],
        );
      } catch (err: any) {
  ...
  }, [token, resolvedClientId, resolvedUserId, normalizedRole]);
