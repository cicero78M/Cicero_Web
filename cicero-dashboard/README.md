...
- Resolusi nama satker di halaman User Directory kini memanggil `getClientNames` tanpa mewariskan context `scope=DIREKTORAT`, sehingga lookup label tetap canonical berdasarkan `client_id` (terutama untuk satker ORG) sementara context role/scope tetap dipakai khusus pada `getUserDirectory` untuk filter data.
...
- Panel aksi rekap likes Instagram kini difokuskan ke alur salin teks ("Salin Teks Rekap" dan "Salin Rekap") tanpa opsi unduh tabel JPG agar API imperative dan kontrol UI tetap sederhana serta konsisten dengan kebutuhan operasional terkini.
...
- Label dropdown selector satker pada `/likes/instagram` untuk scope direktorat kini distandarkan menjadi **"Pilih Client Direktorat / Satker"** agar konsisten dengan halaman User Directory dan User Insight.
...
- Shim kompatibilitas `document.selector` (mengarah ke `document.querySelector`) kini dipasang sedini mungkin di `app/layout.tsx` lewat script inline di `<head>` agar dieksekusi sebelum hydration, lalu tetap dipertahankan sebagai fallback di `LayoutClient`, sehingga skrip eksternal/browser extension yang memanggil API non-standar (contoh `document.selector("#comm1")`) tidak lagi memicu error `Uncaught TypeError` pada halaman insight seperti `/likes/instagram` dan `/comments/tiktok`.
...
- `useInstagramLikesData` kini juga mengekspose `clientOptions` khusus direktorat yang dibangun dari data master `getUserDirectory` (dengan konteks `role`/`scope`/`regional_id` dari auth), lalu dinormalisasi lewat `normalizeUsersWithClientLabel` + `extractClientOptions` dan diperkaya fallback nama klien dari metadata rekap serta `getClientNames`. Dengan begitu selector client pada insight Instagram tidak lagi bergantung pada `chartData`, sehingga opsi Polres tetap lengkap saat grafik sedang terfilter per scope/client.
