...
- `AuthContext` kini mengekstrak dan menyimpan `premium_tier` serta tanggal kedaluwarsa premium (`premium_expiry`/`expires_at`) dari profil client, kemudian membagikannya lewat `useAuth`. Guard baru `hooks/useRequirePremium` memblokir/redirect halaman ANEV bila tier bukan `premium_1` atau `premium_3` dan hanya berjalan setelah `hasResolvedPremium` **serta** `premiumTierReady` bernilai `true`, sehingga akun bertier premium tidak ikut ter-redirect saat profil belum selesai diekstraksi. Menu sidebar selalu mengarah ke rute **/anev/polres** (label tetap menandai status premium ketika akses belum tersedia). Rute lama `/dashboard/anev` otomatis mengarahkan ke halaman ANEV agar tautan lama tetap berfungsi.
