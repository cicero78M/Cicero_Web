...
- Halaman **/anev/polres** memakai `useRequireAuth` + guard `useRequirePremium` (tier **Premium 1** atau **Premium 3**) dan memanggil `getDashboardAnev` untuk menampilkan snapshot filter, ringkasan agregat (total user, likes, comments, expected actions, serta posting per platform), tabel `compliance_per_pelaksana` dengan completion rate, dan feedback error 400/403 lengkap CTA daftar premium.
- Normalisasi ANEV Polres kini membaca breakdown user per satfung/divisi dari `aggregates.raw`, `aggregates.totals`, ataupun payload `raw` yang dikirim langsung oleh API. Bentuk yang diterima bisa berupa array objek `{ satfung|division|divisi|unit|name|label, count|total|users|user|value|jumlah|personel }` maupun map objek `{ \"SATFUNG\": 12 }` atau `{ key: { satfung: \"SATFUNG\", count: 12 } }`. Semua variasi dikonversi menjadi array `{ label, count }` sebelum dirender.
- Segmen **User per Satfung/Divisi** di halaman ANEV Polres menampilkan kartu grid label + jumlah user dan otomatis menampilkan empty state “Belum ada data satfung/divisi...” jika array hasil normalisasi kosong agar UI tidak memunculkan kartu kosong.
...
- Filter waktu **/anev/polres** kini berbentuk segmented pill (Today, This Week, This Month, Custom). Preset mingguan/bulanan otomatis menghitung `start_date`/`end_date` sebelum memanggil `getDashboardAnev` sembari tetap mengirim `time_range` ke API; snapshot filter menampilkan label preset aktif. Tombol apply dinonaktifkan saat rentang custom belum lengkap.
...
- `AuthContext` kini mengekstrak dan menyimpan `premium_tier` serta tanggal kedaluwarsa premium (`premium_expiry`/`expires_at`) dari profil client, kemudian membagikannya lewat `useAuth`. Guard baru `hooks/useRequirePremium` memblokir/redirect halaman ANEV bila tier bukan `premium_1` atau `premium_3`, sementara menu sidebar menampilkan entri **Anev Polres** ke `/anev/polres` hanya untuk tier yang memenuhi dan CTA ke `/premium/anev` bila belum berlangganan (terpisah dari menu Premium umum `/premium`). Rute lama `/dashboard/anev` otomatis mengarahkan ke halaman ANEV agar tautan lama tetap berfungsi.
- Nilai tier premium dinormalisasi ke huruf kecil serta menghapus spasi, garis bawah, maupun tanda hubung sehingga variasi seperti `Premium 1`, `premium-1`, `premium_1`, dan `premium1` dianggap setara. Akses ANEV Polres diberikan untuk tier `premium1` atau `premium3`; nilai lain akan diarahkan ke `/premium`, dan menu sidebar mengikuti logika yang sama.
