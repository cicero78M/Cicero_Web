...
- `/reposter/tasks/official` kini mengambil posting Instagram dari `GET /api/insta/posts?client_id=...`, menyaring konten "hari ini" (waktu lokal), lalu mengurutkannya berdasarkan `created_at` agar konten terbaru muncul lebih dulu.
- `/reposter/profile` kini mengambil detail user dari tabel `user` melalui endpoint Cicero_V2 (`GET /api/users/{nrp}`), menampilkan Client ID, dan menyediakan form edit untuk Nama, Pangkat, Satfung, Jabatan, username Instagram/TikTok, serta email dengan tombol simpan yang memanggil `PUT /api/users/{nrp}`.
